...
(function($) {
	...
	var imgCheckboxClass = function(element, options, id) {
		...
		/* *** DOM STUFF *** */
		element.wrap("<span class='imgChkWrapper imgCheckbox" + id + "'>");
		$wrapperElement = element.parent();
		$selectElement = element.find('.btn-select a');
		...
		$selectElement.click(function() {
			changeSelection($(this).parents('.img-layout-list').parent(), CHK_TOGGLE, options.addToForm, options.radio, options.canDeselect, $wrapperElement);
			if (options.onclick)
				options.onclick($(this).parent());
		});
